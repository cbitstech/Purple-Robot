...
public class JavaScriptEngine extends BaseScriptEngine {
    ...
    public Object runScript(String script, String extrasName, Object extras)
        ...
        if (extras instanceof Map<?, ?>) {
            extras = JavaScriptEngine.mapToNative(this._jsContext, this._scope,
                    (Map<String, Object>) extras);

            extras = NativeJSON.stringify(this._jsContext, this._scope, extras,
                    null, null);
        }

        if (extras != null && extrasName != null)
            script = "var " + extrasName + " = " + extras.toString() + "; "
                    + script;

        return this._jsContext.evaluateString(this._scope, script, "<engine>",
                0, null);
    }

    public boolean log(String event, NativeObject params) {
        if (params != null)
            return LogManager.getInstance(this._context).log(event,
                    JavaScriptEngine.nativeToMap(params));
    ...
    public boolean loadLibrary(String libraryName) {
        if (this._jsContext != null && this._scope != null) {
            try {
                ...
                this._jsContext.evaluateString(this._scope, script, "<engine>",
                        1, null);
    ...
    public boolean broadcastIntent(final String action,
            final NativeObject extras) {
        return this.broadcastIntent(action,
                JavaScriptEngine.nativeToMap(extras));
    }

    public boolean updateWidget(final String title, final String message,
            final String applicationName, final NativeObject launchParams,
            final String script) {
    ...
    public boolean showApplicationLaunchNotification(String title,
            ...
            boolean persistent, final NativeObject launchParams,
            final String script) {
        return this.showApplicationLaunchNotification(title, message,
                applicationName, displayWhen, persistent,
                JavaScriptEngine.nativeToMap(launchParams), script);
    }
    ...
    public static JSONArray nativeToJson(NativeArray nativeArray)
    ...
    public static JSONObject nativeToJson(NativeObject nativeObj)
            throws JSONException {
        if (nativeObj == null)
            return null;
    ...
    @SuppressWarnings("unchecked")
    private static NativeObject mapToNative(Context context, Scriptable scope,
        ...
        for (String key : map.keySet()) {
            ...
            if (value instanceof Map)
                value = JavaScriptEngine.mapToNative(context, scope,
    ...
    public NativeObject fetchSnapshot(String timestamp) {
        ...
        if (snapshot != null)
            return JavaScriptEngine.mapToNative(this._jsContext, this._scope,
                    snapshot);
    ...
    public NativeObject models() {
        Map<String, Object> modelMap = ModelManager.getInstance(this._context)
                .models(this._context);
    ...
    public NativeObject readings() {
        Map<String, Object> readings = ModelManager.getInstance(this._context)
                .readings(this._context);
    ...
    public NativeObject predictions() {
        Map<String, Object> predictions = ModelManager.getInstance(
                this._context).predictions(this._context);
