...
public class HttpStoreRequestHandler implements HttpRequestHandler {
    ...
    public void handle(HttpRequest request, HttpResponse response,
        ...
        if (request instanceof HttpEntityEnclosingRequest) {
            ...
            try {
                for (NameValuePair pair : URLEncodedUtils.parse(
                        new URI(u.toString()), "UTF-8")) {
            ...
            if (arguments.size() > 0) {
                ...
                if (arguments.containsKey("encrypt")
                        && "true".equals(arguments.get("encrypt")))
                    encrypt = true;
                ...
                for (String key : arguments.keySet()) {
                    if ("encrypt".equalsIgnoreCase(key)
                            || "namespace".equalsIgnoreCase(key)) {
                        // Ignore these values...
                    } else {
                        ...
                        if (value.length() > 0) {
                            if (encrypt) {
                                if (namespace != null)
                                    engine.persistEncryptedString(namespace,
                                            key, value);
                                else
            ...
            JSONObject obj = JsonVariablesRequestHandler
                    .fetchStoredValues(this._context);
            ...
            try {
                ...
                StringEntity body = new StringEntity(
                        this._context
                                .getString(R.string.error_malformed_request));
