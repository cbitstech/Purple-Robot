...
public class ConfigurationCheck extends SanityCheck {
    ...
    public void runCheck(Context context) {
        ...
        long refresh = Long.parseLong(prefs.getString(
                "config_json_refresh_interval", "0"));

        if (refresh > 0 && refresh < 600) {
            this._errorLevel = SanityCheck.WARNING;
            this._errorMessage = context
                    .getString(R.string.name_sanity_configuration_refresh_warning);
        ...
        if (prefs.getBoolean("config_enable_data_server", false)) {
            long upload = Long.parseLong(prefs.getString(
                    "config_http_upload_interval", "0"));

            if (upload > 0 && upload < 300000) {
                this._errorLevel = SanityCheck.WARNING;
                this._errorMessage = context
                        .getString(R.string.name_sanity_configuration_upload_warning);
