...
public class SchemeConfigCheck extends SanityCheck {
    ...
    public void runCheck(Context context) {
        ...
        if (prefs.contains("scheme_config_contents")
                && prefs.getBoolean("check_scheme_config", true)) {
            String schemeScript = prefs.getString("scheme_config_contents",
                    "()");

            Evaluator eval = new Evaluator();
            eval.getInteractionEnvironment().setValue(
                    Symbol.intern("PurpleRobot"), this);
            eval.getInteractionEnvironment().setValue(
                    Symbol.intern("JSONHelper"), new JSONHelper());
            JScheme scheme = new JScheme(eval);

            try {
                scheme.load(new InputStreamReader(context.getAssets().open(
                        "scheme/pregexp.scm")));
            } catch (IOException e) {
            ...
            try {
                ...
                if (sexp instanceof Pair) {
                    ...
                    List<Map<String, Object>> configs = SchemeConfigCheck
                            .parseConfigMaps(pair);
                            ...
                            for (String key : config.keySet()) {
                                if (prefs.contains(key)) {
                                    ...
                                    if (cfgObject instanceof Boolean) {
                                        ...
                                        boolean pref = prefs.getBoolean(key,
                                                (setting == false));
                                        ...
                                        if (pref != setting) {
                                            ...
                                            this._errorMessage = context
                                                    .getString(
                                                            R.string.scheme_config_check_changed,
                                                            key);
                                        }
                                    ...
                                    this._errorMessage = context
                                            .getString(
                                                    R.string.scheme_config_check_missing,
                                                    key);
                ...
                this._errorLevel = SanityCheck.ERROR;
                this._errorMessage = context
                        .getString(R.string.scheme_config_invalid_config);
    ...
    private static List<Map<String, Object>> parseConfigMaps(Pair pair) {
        ...
        if (first instanceof Symbol
                && "pr-update-config".equals(first.toString())
                && rest instanceof Pair) {
