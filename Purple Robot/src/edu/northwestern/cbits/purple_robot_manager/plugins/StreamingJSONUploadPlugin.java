...
public class StreamingJSONUploadPlugin extends DataUploadPlugin {
                    ...
                    String[] filenames = pendingFolder
                            .list(new FilenameFilter() {
                                public boolean accept(File dir, String filename) {
                                    return filename
                                            .endsWith(StreamingJSONUploadPlugin.FILE_EXTENSION);
                                }
                            });
    ...
    public void processIntent(final Intent intent) {
        ...
        if (OutputPlugin.FORCE_UPLOAD.equals(action)) {
            ...
            try {
                ...
                if (this._writer == null) {
                    this._currentFile = new File(this.getPendingFolder(), now
                            + FILE_EXTENSION);

                    BufferedOutputStream out = new BufferedOutputStream(
                            new FileOutputStream(this._currentFile));
                    this._writer = new JsonWriter(new OutputStreamWriter(out,
                            "UTF-8"));

                    this._writer.beginArray();
                }

                StreamingJSONUploadPlugin.writeBundle(this.getContext(),
                        this._writer, extras);
                this._writer.flush();
            } catch (IOException e) {
    ...
    public static void writeBundle(Context context, JsonWriter writer,
            Bundle bundle) {
        try {
            writer.beginObject();
                    ...
                    if (value instanceof String) {
                        ...
                        for (Object o : list) {
                            if (o instanceof String)
                                ...
                                StreamingJSONUploadPlugin.writeBundle(context,
                                        writer, (Bundle) o);
                                ...
                                if (s.capabilities != null)
                                    writer.name("Capabilities").value(
                                            s.capabilities);
                                ...
                                if (r.baseActivity != null)
                                    writer.name("Base Activity").value(
                                ...
                                writer.name("Activity Count").value(
                                        r.numActivities);
                                writer.name("Running Activity Count").value(
                                        r.numRunning);
                                ...
                                writer.name("Longitude")
                                        .value(l.getLongitude());

                                if (l.getProvider() != null)
                                    writer.name("Provider").value(
                                            l.getProvider());
                                else
                        ...
                        writer.name("Device Address")
                                .value(device.getAddress());
                        writer.name("Device Class").value(
                                device.getBluetoothClass().toString());

                        writer.endObject();
                    } else if (value instanceof Bundle)
                        StreamingJSONUploadPlugin.writeBundle(context, writer,
                                (Bundle) value);
                    else
                        Log.e("PRM", "GOT TYPE "
