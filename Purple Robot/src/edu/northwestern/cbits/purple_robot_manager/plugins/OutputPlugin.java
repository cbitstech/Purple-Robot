...
public abstract class OutputPlugin {
    ...
    public static void loadPluginClasses(Context context) {
        ...
        LocalBroadcastManager localManager = LocalBroadcastManager
                .getInstance(context);
        localManager.registerReceiver(OutputPluginManager.sharedInstance,
                intentFilter);
    }
    ...
    public static JSONObject jsonForBundle(Bundle bundle) throws JSONException {
        ...
        for (String key : values.keySet()) {
            ...
            if (value == null || key == null) {
                ...
                for (Object o : list) {
                    if (o instanceof Bundle)
                        ...
                        if (r.baseActivity != null)
                            taskObject.put("Base Activity",
                                    r.baseActivity.getPackageName());

                        if (r.description != null)
                            taskObject.put("Description",
                        ...
                        Log.e("PRM", "LIST OBJ: "
                                + o.getClass().getCanonicalName() + " IN "
                                + key);
                }
                ...
                deviceObject.put("Device Class", device.getBluetoothClass()
                        .toString());

                json.put(key, deviceObject);
            } else
                Log.e("PRM", "GOT TYPE " + value.getClass().getCanonicalName()
                        + " FOR " + key);
        }
        ...
        try {
            ...
            while (keys.hasNext()) {
                ...
                if (o instanceof Double)
                    ...
                    if (child.length() > 0) {
                        ...
                        if (nextChild instanceof String) {
                            ...
                            for (int i = 0; i < child.length(); i++)
                                bundles.add(OutputPlugin.bundleForJson(context,
                                        child.getJSONObject(i).toString()));
                    ...
                    bundle.putBundle(
                            key,
                            OutputPlugin.bundleForJson(context,
                                    child.toString()));
                }
            }
