...
public abstract class XYZBasicFrequencyFeature extends ContinuousProbeFeature {
    ...
    private void appendValues(float[] incomingX, float[] incomingY,
            float[] incomingZ, double[] ts) {
        ...
        for (int i = 0; i < this._currentIndex; i++) {
            readings.add(new Reading(_timestamps[i], _xValues[i], _yValues[i],
                    _zValues[i]));
        }
    ...
    protected void processData(final Context context, Bundle dataBundle) {
        if (dataBundle.containsKey("EVENT_TIMESTAMP")
                && dataBundle.containsKey("X") && dataBundle.containsKey("Y")
            ...
            long updateInterval = Long.parseLong(prefs.getString(
                                                         ...
                                                         // check for config
            {
                ...
                int twoPow = ts.length == 0 ? 0 : (32 - Integer
                        .numberOfLeadingZeros(ts.length - 1));
                ...
                for (int i = 0; i < _interX.length; i++) {
                    if (i < 2) {
                        ...
                        if (i == _dynamicX.length - 1) {
                            _dynamicX[i] = XYZBasicFrequencyFeature.bpFilter(
                                    _interX, this._xBPHistory, i, "X");
                            _dynamicY[i] = XYZBasicFrequencyFeature.bpFilter(
                                    _interY, this._yBPHistory, i, "Y");
                            _dynamicZ[i] = XYZBasicFrequencyFeature.bpFilter(
                                    _interZ, this._zBPHistory, i, "Z");

                            _staticX[i] = XYZBasicFrequencyFeature.lpFilter(
                                    _interX, this._xLPHistory, i, "X");
                            _staticY[i] = XYZBasicFrequencyFeature.lpFilter(
                                    _interY, this._yLPHistory, i, "Y");
                            _staticZ[i] = XYZBasicFrequencyFeature.lpFilter(
                                    _interZ, this._zLPHistory, i, "Z");
                        } else {
                            _dynamicX[i] = XYZBasicFrequencyFeature.bpFilter(
                                    _interX, this._xBPHistory, i, null);
                            _dynamicY[i] = XYZBasicFrequencyFeature.bpFilter(
                                    _interY, this._yBPHistory, i, null);
                            _dynamicZ[i] = XYZBasicFrequencyFeature.bpFilter(
                                    _interZ, this._zBPHistory, i, null);

                            _staticX[i] = XYZBasicFrequencyFeature.lpFilter(
                                    _interX, this._xLPHistory, i, null);
                            _staticY[i] = XYZBasicFrequencyFeature.lpFilter(
                                    _interY, this._yLPHistory, i, null);
                            _staticZ[i] = XYZBasicFrequencyFeature.lpFilter(
                                    _interZ, this._zLPHistory, i, null);
                        }
                ...
                Complex[] xFFT = fft
                        .transform(_dynamicX, TransformType.FORWARD);
                Complex[] yFFT = fft
                        .transform(_dynamicY, TransformType.FORWARD);
                Complex[] zFFT = fft
                        .transform(_dynamicZ, TransformType.FORWARD);

                double[] frequencies = XYZBasicFrequencyFeature
                        .calculateFreqArray(_interX.length, observedFreq);

                final double[] _xMaxFreqPowPair = XYZBasicFrequencyFeature
                        .findPeakFrequency(xFFT, frequencies);
                final double[] _yMaxFreqPowPair = XYZBasicFrequencyFeature
                        .findPeakFrequency(yFFT, frequencies);
                final double[] _zMaxFreqPowPair = XYZBasicFrequencyFeature
                        .findPeakFrequency(zFFT, frequencies);
                ...
                Runnable r = new Runnable() {
                    public void run() {
                        ...
                        boolean incInterpolated = prefs
                                .getBoolean(
                                        "config_probe_" + key
                                                + "_interpolated_enabled",
                                        XYZBasicFrequencyFeature.INTERPOLATED_ENABLED);
                        boolean incBandpass = prefs.getBoolean("config_probe_"
                                + key + "_bandpass_enabled",
                                XYZBasicFrequencyFeature.BANDPASS_ENABLED);
                        boolean incLowpass = prefs.getBoolean("config_probe_"
                                + key + "_lowpass_enabled",
                        ...
                        if (incInterpolated || incBandpass || incLowpass) {
                            ...
                            synchronized (me) {
                                sensorData.putDoubleArray("INTERP_TIMESTAMPS",
                                        interTimes);

                                if (incInterpolated) {
                                    sensorData.putDoubleArray("INTER_X",
                                            _interX);
                                    sensorData.putDoubleArray("INTER_Y",
                                            _interY);
                                    sensorData.putDoubleArray("INTER_Z",
                                            _interZ);
                                }

                                if (incBandpass) {
                                    sensorData.putDoubleArray("DYNAMIC_X",
                                            _dynamicX);
                                    sensorData.putDoubleArray("DYNAMIC_Y",
                                            _dynamicY);
                                    sensorData.putDoubleArray("DYNAMIC_Z",
                                            _dynamicZ);
                                }

                                if (incLowpass) {
                                    sensorData.putDoubleArray("STATIC_X",
                                            _staticX);
                                    sensorData.putDoubleArray("STATIC_Y",
                                            _staticY);
                                    sensorData.putDoubleArray("STATIC_Z",
                                            _staticZ);
                                }
    ...
    private static double[] findPeakFrequency(Complex[] samples,
            double[] frequencies) {
        ...
        if (index >= 0) {
            ...
            returnFrequencyPowerPair[POWER_INDEX] = 2 * Math.abs(samples[index]
                    .getReal());
        }
    ...
    private static double[] calculateFreqArray(int numSamples,
            double maxFrequency) {
    ...
    public static double bpFilter(double[] inputs, double[] outputs,
    ...
    public static double lpFilter(double[] inputs, double[] outputs,
            int offset, String label) {
        // Magic numbers: M is for MatLab...

        double[] beta = { 0.00024135904904198073, 0.0004827180980838,
    ...
    private static double[] lowPassFilter(double[] values, double interval,
        ...
        for (int i = 0; i < newValues.length; i++) {
            last = XYZBasicFrequencyFeature.weightedSmoothing(values[i], last,
                    a);
    ...
    public PreferenceScreen preferenceScreen(PreferenceActivity activity) {
        ...
        interpolated
                .setDefaultValue(XYZBasicFrequencyFeature.INTERPOLATED_ENABLED);
    ...
    public String summarizeValue(Context context, Bundle bundle) {
        ...
        return String.format(
                context.getResources().getString(
                        R.string.summary_frequency_statistics_feature), freqX,
