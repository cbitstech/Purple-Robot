...
public class ProximityProbe extends ContinuousProbe implements
        SensorEventListener {
    ...
    public String contentSubtitle(Context context) {
        Cursor c = ProbeValuesProvider.getProvider(context).retrieveValues(
                context, ProximityProbe.DB_TABLE, this.databaseSchema());
    ...
    public String getDisplayContent(Activity activity) {
        try {
            ...
            Cursor cursor = ProbeValuesProvider.getProvider(activity)
                    .retrieveValues(activity, ProximityProbe.DB_TABLE,
                            this.databaseSchema());
            ...
            if (cursor != null) {
                ...
                while (cursor.moveToNext()) {
                    double d = cursor.getDouble(cursor
                            .getColumnIndex(ProximityProbe.DISTANCE_KEY));
        ...
        if (distance != null && eventTimes != null) {
            ...
            if (eventTimes.length > 1) {
                ...
                for (int i = 0; i < eventTimes.length; i++) {
                    String formatString = context
                            .getString(R.string.proximity_formatted);
    ...
    public long getFrequency() {
        ...
        return Long.parseLong(prefs.getString(
                "config_probe_proximity_built_in_frequency",
    ...
    public boolean isEnabled(Context context) {
        ...
        if (super.isEnabled(context)) {
            if (prefs.getBoolean("config_probe_proximity_built_in_enabled",
    ...
    public void updateFromMap(Context context, Map<String, Object> params) {
        ...
        if (params.containsKey(ContinuousProbe.PROBE_THRESHOLD)) {
            ...
            if (threshold instanceof Double) {
                ...
                e.putString("config_probe_proximity_threshold",
                        threshold.toString());
                e.commit();
            }
        }
    ...
    protected boolean passesThreshold(SensorEvent event) {
        ...
        if (now - this.lastThresholdLookup > 5000) {
            ...
            this.lastThreshold = Double.parseDouble(prefs.getString(
                    "config_probe_proximity_threshold",
            ...
            synchronized (this) {
                ...
                if (bufferIndex >= timeBuffer.length) {
                    ...
                    for (int j = 0; j < timeBuffer.length; j++) {
                        ...
                        for (int i = 0; i < fieldNames.length; i++) {
                            if (fieldNames[i]
                                    .equals(ProximityProbe.DISTANCE_KEY))
    ...
    public String summarizeValue(Context context, Bundle bundle) {
        ...
        return String.format(
                context.getResources().getString(
                        R.string.summary_proximity_value_probe), distance);
    }
