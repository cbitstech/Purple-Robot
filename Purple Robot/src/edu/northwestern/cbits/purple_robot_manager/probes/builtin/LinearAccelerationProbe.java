...
public class LinearAccelerationProbe extends ContinuousProbe implements
        SensorEventListener {
    ...
    public String contentSubtitle(Context context) {
        Cursor c = ProbeValuesProvider.getProvider(context).retrieveValues(
                context, LinearAccelerationProbe.DB_TABLE,
                this.databaseSchema());
    ...
    public Map<String, String> databaseSchema() {
        if (this._schema == null) {
            ...
            this._schema.put(LinearAccelerationProbe.X_KEY,
                    ProbeValuesProvider.REAL_TYPE);
            this._schema.put(LinearAccelerationProbe.Y_KEY,
                    ProbeValuesProvider.REAL_TYPE);
            this._schema.put(LinearAccelerationProbe.Z_KEY,
                    ProbeValuesProvider.REAL_TYPE);
        }
    ...
    public String getDisplayContent(Activity activity) {
        try {
            ...
            Cursor cursor = ProbeValuesProvider.getProvider(activity)
                    .retrieveValues(activity, LinearAccelerationProbe.DB_TABLE,
                            this.databaseSchema());
            ...
            if (cursor != null) {
                ...
                while (cursor.moveToNext()) {
                    double xd = cursor.getDouble(cursor
                            .getColumnIndex(LinearAccelerationProbe.X_KEY));
                    double yd = cursor.getDouble(cursor
                            .getColumnIndex(LinearAccelerationProbe.Y_KEY));
                    double zd = cursor.getDouble(cursor
                            .getColumnIndex(LinearAccelerationProbe.Z_KEY));
    ...
    public Bundle formattedBundle(Context context, Bundle bundle) {
        ...
        if (x == null || y == null || z == null) {
            ...
            formatted.putBundle(context
                    .getString(R.string.display_linear_acceleration_readings),
                    readings);
    ...
    public long getFrequency() {
        ...
        return Long.parseLong(prefs.getString(
                "config_probe_linear_acceleration_built_in_frequency",
    ...
    public boolean isEnabled(Context context) {
        ...
        if (super.isEnabled(context)) {
            if (prefs.getBoolean(
                    "config_probe_linear_acceleration_built_in_enabled",
    ...
    protected boolean passesThreshold(SensorEvent event) {
        ...
        if (now - this.lastThresholdLookup > 5000) {
            ...
            this.lastThreshold = Double.parseDouble(prefs.getString(
                    "config_probe_linear_acceleration_threshold",
    ...
    public void updateFromMap(Context context, Map<String, Object> params) {
        ...
        if (params.containsKey(ContinuousProbe.PROBE_THRESHOLD)) {
            ...
            if (threshold instanceof Double) {
                ...
                e.putString("config_probe_linear_acceleration_threshold",
                        threshold.toString());
                e.commit();
            }
        }
            ...
            synchronized (this) {
                ...
                if (bufferIndex >= timeBuffer.length) {
                    ...
                    for (int j = 0; j < timeBuffer.length; j++) {
                        ...
                        for (int i = 0; i < fieldNames.length; i++) {
                            if (fieldNames[i]
                                    .equals(LinearAccelerationProbe.X_KEY))
                                x = Double.valueOf(valueBuffer[i][j]);
                            else if (fieldNames[i]
                                    .equals(LinearAccelerationProbe.Y_KEY))
                                y = Double.valueOf(valueBuffer[i][j]);
                            else if (fieldNames[i]
                                    .equals(LinearAccelerationProbe.Z_KEY))
                        ...
                        if (x != null && y != null && z != null) {
                            ...
                            ProbeValuesProvider.getProvider(this._context)
                                    .insertValue(this._context,
                                            LinearAccelerationProbe.DB_TABLE,
    ...
    public String summarizeValue(Context context, Bundle bundle) {
        ...
        return String.format(
                context.getResources().getString(
                        R.string.summary_linear_acceleration_probe), xReading,
                yReading, zReading);
    }
