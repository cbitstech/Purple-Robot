...
import edu.northwestern.cbits.purple_robot_manager.probes.features.p20.ExternalAppHelperProbe;
...
public class ProbeManager
{
    ...
    public static Probe probeForName(String name, Context context)
    {
        ...
        for (Probe probe : ProbeManager.allProbes(context))
        {
            ...
            else if (probe instanceof ExternalAppHelperProbe)
            {
                ExternalAppHelperProbe external = (ExternalAppHelperProbe) probe;

                if (external.name(context).equalsIgnoreCase(name))
                    found = true;
            }
