 ...
 * 
 * In addition to implementing this class, to register the new Probe with 
 * Purple Robot, the following steps must be completed:
 * 
 ...
 *    matches the given name. (This is an old & suboptimal mechanism that is
 *    due for replacement as time becomes available.)
 *    
...
public class SampleProbe extends Probe
{
	 ...
	 * system. Typically is the full class name of the probe and defined as
	 * a class constant for use elsewhere in the system where a Context 
	 * object may be unavailable. 
	 */
	@Override
	...
	/**
	 * Returns a category name used to group probes in the app settings.
	 * Names should be defined in `strings.xml`, and not the source code 
	 * for translations.
	 */
	@Override
	...
	/**
	 * Generates and returns a preferences screen for setting the options
	 * for the probe. The title and summary are shown in the probe lists.
	 * All probes should include an "Enable" checkbox to allow the user
	 * to deactivate the probe as needed. Additional optins such as
	 * frequencies and durations may also be defined. In the case below, 
	 * a frequency parameter is defined that dictates how often the probe
	 * should emit a reading.
	 */
	...
	public PreferenceScreen preferenceScreen(PreferenceActivity activity) 
	{
		...
		// Adding a frequency parameter that limits how frequently the probe can sample data...
	...
	/**
	 * Returns a human-readable summary that explains what the probe is and
	 * what it does.
	 */
	@Override
	...
	/**
	 * Called by the system every fifteen seconds to verify if the probe is enabled and 
	 * if there's work to do. Note that if a larger sample rate is required, probes must 
	 * set up and manage their own threads that implement the desired sample rate.
	 * 
	 * (The naming of this method needs to be revisited and revised.)
	 * 
	 * This example will illustrate how to communicate with the device's built-in 
	 * audio capture devices to calculate audio features from regularly-recorded audio
	 * samples. 
	 */

	public boolean isEnabled(final Context context)
	{
		// First, check if probes are enabled across the app (Settings->Probes->Enable Probes)
		
		if (super.isEnabled(context))
		{
			// Probes are enabled system-wide, continue...
			
			// Fetch the SharedPreferences object of obtaining configuration parameters...
			...
			// Fetch the frequency settings and date to determine if enough time has elapsed
			...
			if (this._recording == false)
			{
				...
				// Since we are going to collect and analyze data in separate threads, create
				// a final pointer to this instance of the probe so that we can still access
				// functions like emitReading to send the data to the rest of the system after
				...
				Runnable r = new Runnable()
				{
					...
					public void run() 
					{
						// Determine the size of the smallest buffer the probe will need
						...
						// Find the highest sample rate the device supports and create an
						...
						// If we were able to find a sample rate and create an AudioRecorder
						// instance...
						
						if (recorder != null)
						{
							// Create a buffer for samples and start recording. Note that the recording duration
							// is a function of the sample rate and master buffer size (in seconds):
							//
							...
							// Designate a place to store a running tally for calculating
							// the signal power...
							
							double sampleSum = 0;
							double samplePower = 0;
							
							// While we have room in the master buffer and audio is available...
							...
							// Start building the data payload that the probe will emit. Note that the timestamp
							...
							// The AudioRecord instance is no longer needed at the moment, free it...
														
							recorder.release();

							// Let's calculate the magnitude and frequency of the audio signal using a 
							// Fast Fourier Transform. (Implementation provided by the Apache Commons Math
							// library.)
							// 
							// If you wanted to calculate your own features, you would replace the code below
							// with your own code for analyzing the collected samples.
							...
							// Look at the possible wavelengths and determine which one has the largest magnitude...
							...
							// Transmit. Note that this is a thread-safe method, so probe data can be transmitted
							// on a separate thread than the original one containing the isEnabled call.
							
							me.transmitData(context, bundle);
						}

						// Wait a moment while data is flushed out of the system before we allow recording to 
						// resume...
						...
						// Reset the flag to allow recording to start in the next cycle.
	...
	/**
	 * Provides a human-readable display of the data in places like the main screen. A copy of the bundle
	 * transmitted is provided, and this method returns a string formatting the most relevant details.
	 */
