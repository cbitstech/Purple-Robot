...
public class MatlabTreeModelTestCase extends RobotTestCase {
    ...
    public void test() {
        ...
        try {
            ...
            InputStream file = this._context.getAssets().open(
                    "test_data/matlab-tree.txt");
            ...
            Assert.assertEquals("MATLAB2", "6",
                    prediction.get(LeafNode.PREDICTION));

            Assert.assertEquals(
                    "MATLAB3",
                    "   1  if x10<-0.464422 then node 2 elseif x10>=-0.464422 then node 3 else 8",
                    lines.get(0));
            Assert.assertEquals("MATLAB4", "1815  class = 1", lines.get(1814));
            Assert.assertEquals(
                    "MATLAB5",
                    "1455  if x18<0.26036 then node 1560 elseif x18>=0.26036 then node 1561 else 4",
                    lines.get(1454));
            Assert.assertEquals(
                    "MATLAB6",
                    " 894  if x100<1.42992 then node 1092 elseif x100>=1.42992 then node 1093 else 6",
                    lines.get(893));
            Assert.assertEquals(
                    "MATLAB7",
                    "   3  if x3<0.392665 then node 6 elseif x3>=0.392665 then node 7 else 8",
                    lines.get(2));
            Assert.assertEquals(
                    "MATLAB8",
                    "   9  if x108<-0.0457584 then node 18 elseif x108>=-0.0457584 then node 19 else 6",
                    lines.get(8));
        ...
        Assert.assertNotNull("MATLAB200", models.fetchModelByName(
                this._context, MatlabTreeModelTestCase.MODEL_URI));
        Assert.assertNull("MATLAB201", models.fetchModelByTitle(this._context,
                MatlabTreeModelTestCase.MODEL_URI));
        Assert.assertNotNull("MATLAB202", models.fetchModelByTitle(
                this._context, "Matlab Tree Model Test"));
        ...
        Model matlab = models.fetchModelByTitle(this._context,
                "Matlab Tree Model Test");
        ...
        Assert.assertEquals("MATLAB203", "3",
                matlab.latestPrediction(this._context).get(LeafNode.PREDICTION));
