...
public class LegacyJSONConfigFile {
            ...
            public void run() {
                ...
                if (uri != null && uri.toString().trim().length() > 0) {
                    try {
                        ...
                        final SharedPreferences prefs = LegacyJSONConfigFile
                        ...
                        if (conn instanceof HttpsURLConnection) {
                            ...
                            if (prefs.getBoolean("config_http_liberal_ssl",
                                    true)) {
                        ...
                        for (int z = 0; z < 16
                                && conn.getHeaderField("Location") != null; z++) {
                            URL newUrl = new URL(
                        ...
                        String scriptString = new String(bout.toByteArray(),
                                "UTF-8");

                        String oldHash = prefs.getString(
                                LegacyJSONConfigFile.JSON_LAST_HASH, "");
                        final String newHash = encryption.createHash(context,
                                scriptString);

                        PurpleRobotApplication.fixPreferences(context, true);

                        if (scriptString.toLowerCase().startsWith("(begin ")
                                || (conn.getContentType() != null && conn
                            ...
                            edit.putString("scheme_config_contents",
                                    scriptString);
                            ...
                            if (oldHash.equals(newHash) == false) {
                                try {
                                    ...
                                    SchemeEngine scheme = new SchemeEngine(
                                            context, null);
                            ...
                            if (oldHash.equals(newHash) == false) {
                                edit.putString(
                                        LegacyJSONConfigFile.JSON_CONFIGURATION,
                                        scriptString);
                                edit.commit();
                                    ...
                                    public void run() {
                                        try {
                                            if (json.has(LegacyJSONConfigFile.JSON_INIT_SCRIPT)) {
                                                String script = json
                                                        .getString(LegacyJSONConfigFile.JSON_INIT_SCRIPT);
                    ...
                    LegacyJSONConfigFile._sharedFile = new LegacyJSONConfigFile(
                            context, next);
                } else {
                    final SharedPreferences prefs = LegacyJSONConfigFile
                            .getPreferences(context);
    ...
    protected void updateTriggers(Context context) {
        ...
        TriggerManager.getInstance(context)
                .updateTriggers(context, triggerList);
    }
    ...
    private static SharedPreferences getPreferences(Context context) {
        if (LegacyJSONConfigFile.prefs == null)
            LegacyJSONConfigFile.prefs = PreferenceManager
    ...
    private LegacyJSONConfigFile(Context context, Runnable next) {
        ...
        try {
            this.parameters = new JSONObject(prefs.getString(
                    LegacyJSONConfigFile.JSON_CONFIGURATION, "{}"));

            this.updateSharedPreferences(context);
            this.updateTriggers(context);

            String script = this.parameters
                    .getString(LegacyJSONConfigFile.JSON_INIT_SCRIPT);
    ...
    private void updateSharedPreferences(Context context) {
        ...
        try {
            if (this.parameters.has(LegacyJSONConfigFile.USER_ID))
                userId = this.parameters
                        .getString(LegacyJSONConfigFile.USER_ID);

            if (this.parameters.has(LegacyJSONConfigFile.USER_HASH))
                userHash = this.parameters
                        .getString(LegacyJSONConfigFile.USER_HASH);
        } catch (JSONException e) {
            LogManager.getInstance(context).logException(e);
        }
        ...
        if (this.parameters.has(LegacyJSONConfigFile.FEATURES)) {
            ...
            try {
                JSONArray features = this.parameters
                        .getJSONArray(LegacyJSONConfigFile.FEATURES);

                for (int i = 0; i < features.length(); i++) {
                    JSONObject feature = features.getJSONObject(i);

                    String name = context
                            .getString(R.string.label_unknown_feature);
                    String script = "";
                    String formatter = "";
                    ...
                    ProbeManager.addFeature(name,
                            LegacyJSONConfigFile.toSlug(name), script,
                            formatter, sources, false);
                }
            } catch (JSONException e) {
                LogManager.getInstance(context).logException(e);
            }
    ...
    public static void update(final Context context, final boolean force) {
        Runnable r = new Runnable() {
            public void run() {
                SharedPreferences prefs = LegacyJSONConfigFile
                        .getPreferences(context);

                long lastUpdate = prefs.getLong(
                        LegacyJSONConfigFile.JSON_LAST_UPDATE, 0);
                ...
                if (force
                        || (interval > 0 && now - lastUpdate > 1000 * interval)) {
                    Editor edit = prefs.edit();
                    ...
                    if (LegacyJSONConfigFile._sharedFile != null)
                        LegacyJSONConfigFile._sharedFile
