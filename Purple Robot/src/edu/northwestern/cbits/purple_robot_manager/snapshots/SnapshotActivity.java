...
public class SnapshotActivity extends ActionBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Uri queryUri = ContentUris.withAppendedId(
                RobotContentProvider.SNAPSHOTS,
                this.getIntent().getLongExtra("id", 0));

        Cursor c = this.getContentResolver().query(queryUri, null, null, null,
                null);
        ...
        if (c.moveToNext()) {
            ...
            this.getSupportActionBar().setTitle(
                    source + ": " + sdf.format(new Date(recorded)));

            try {
                JSONArray items = new JSONArray(jsonString);

                this.getSupportActionBar().setSubtitle(
                        this.getString(R.string.snapshot_probe_count,
                                items.length()));
        ...
        ArrayAdapter<JSONObject> adapter = new ArrayAdapter<JSONObject>(this,
                R.layout.layout_probe_row, jsonObjects) {
            public View getView(int position, View convertView, ViewGroup parent) {
                if (convertView == null) {
                    LayoutInflater inflater = LayoutInflater.from(me);
                    convertView = inflater.inflate(R.layout.layout_probe_row,
                            parent, false);
                }
                ...
                try {
                    Bundle value = OutputPlugin.bundleForJson(me, json
