...
public class SnapshotManager {
    ...
    public static SnapshotManager getInstance(Context context) {
        if (SnapshotManager._instance == null) {
            SnapshotManager._instance = new SnapshotManager(
    ...
    public long takeSnapshot(Context context, String source, final Runnable r)
            throws EmptySnapshotException {
        final long now = System.currentTimeMillis();

        Cursor cursor = this._context.getContentResolver().query(
        ...
        final Runnable save = new Runnable() {
            public void run() {
                me._context.getContentResolver().insert(
                        RobotContentProvider.SNAPSHOTS, values);
        ...
        if (this._isRecording == false
                && prefs.getBoolean(SnapshotManager.RECORD_AUDIO,
                        SnapshotManager.DEFAULT_RECORD)) {
            ...
            if (context instanceof Activity) {
                ...
                a.runOnUiThread(new Runnable() {
                    public void run() {
                        Toast.makeText(a, R.string.toast_recording_audio,
            ...
            Runnable record = new Runnable() {
                public void run() {
                    ...
                    File audioFolder = new File(internalStorage,
                            "Snapshot Audio");
    ...
    public long[] snapshotTimes() {
        Cursor c = this._context.getContentResolver().query(
                RobotContentProvider.SNAPSHOTS, null, null, null, "recorded");
    ...
    public JSONObject jsonForTime(long time, boolean includeValues) {
        ...
        Cursor c = this._context.getContentResolver().query(
                RobotContentProvider.SNAPSHOTS, null, selection, args, null);
            ...
            try {
                ...
                if (includeValues) {
                    ...
                    for (int i = 0; i < values.length(); i++) {
                        ...
                        Probe p = ProbeManager
                                .probeForName(name, this._context);
    ...
    public void deleteSnapshot(long timestamp) {
        ...
        this._context.getContentResolver().delete(
                RobotContentProvider.SNAPSHOTS, selection, args);
    }
}
