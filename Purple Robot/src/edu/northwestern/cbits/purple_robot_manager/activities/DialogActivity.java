...
public class DialogActivity extends Activity {
    ...
    public static void showNativeDialog(final Context context,
        ...
        if (title == null) {
            DialogActivity.showNativeDialog(context, "", message, confirmTitle,
                    cancelTitle, confirmScript, cancelScript, tag, priority);

            return;
        }

        if (message == null) {
            DialogActivity.showNativeDialog(context, title, "", confirmTitle,
                    cancelTitle, confirmScript, cancelScript, tag, priority);
        ...
        if (confirmTitle == null) {
            DialogActivity.showNativeDialog(context, title, message, "",
                    cancelTitle, confirmScript, cancelScript, tag, priority);
        ...
        if (confirmScript == null) {
            DialogActivity.showNativeDialog(context, title, message,
                    confirmTitle, cancelTitle, "", cancelScript, tag, priority);
        ...
        if (cancelTitle == null) {
            DialogActivity.showNativeDialog(context, title, message,
                    confirmTitle, "", confirmScript, cancelScript, tag,
                    priority);
        ...
        if (cancelScript == null) {
            DialogActivity
                    .showNativeDialog(context, title, message, confirmTitle,
                            cancelTitle, confirmScript, "", tag, priority);
        ...
        if (tag == null || "".equals(tag)) {
            DialogActivity.showNativeDialog(context, title, message,
                    confirmTitle, cancelTitle, confirmScript, cancelScript,
                    UUID.randomUUID().toString(), priority);
        ...
        if (DialogActivity._visible == false) {
            ...
            intent.putExtra(DialogActivity.DIALOG_ADDED,
            ...
            Runnable r = new Runnable() {
                public void run() {
                    ...
                    if (intent.getLongExtra(DialogActivity.DIALOG_PRIORITY, 0) < priority) {
                        ...
                        dialog.put(DialogActivity.DIALOG_TITLE, intent
                                .getStringExtra(DialogActivity.DIALOG_TITLE));
                        dialog.put(DialogActivity.DIALOG_MESSAGE, intent
                                .getStringExtra(DialogActivity.DIALOG_MESSAGE));
                        dialog.put(
                                DialogActivity.DIALOG_CONFIRM_BUTTON,
                                intent.getStringExtra(DialogActivity.DIALOG_CONFIRM_BUTTON));
                        dialog.put(
                                DialogActivity.DIALOG_CONFIRM_SCRIPT,
                                intent.getStringExtra(DialogActivity.DIALOG_CONFIRM_SCRIPT));
                        dialog.put(
                                DialogActivity.DIALOG_CANCEL_BUTTON,
                                intent.getStringExtra(DialogActivity.DIALOG_CANCEL_BUTTON));
                        dialog.put(
                                DialogActivity.DIALOG_CANCEL_SCRIPT,
                                intent.getStringExtra(DialogActivity.DIALOG_CANCEL_SCRIPT));

                        dialog.put(DialogActivity.DIALOG_TAG, intent
                                .getStringExtra(DialogActivity.DIALOG_TAG));
                        dialog.put(DialogActivity.DIALOG_ADDED, intent
                                .getLongExtra(DialogActivity.DIALOG_ADDED,
                                        System.currentTimeMillis()));
                        dialog.put(DialogActivity.DIALOG_PRIORITY,
                                intent.getLongExtra(
                                        DialogActivity.DIALOG_PRIORITY, 0));
    ...
    private void showNativeDialog() {
        ...
        String confirmTitle = intent
                .getStringExtra(DialogActivity.DIALOG_CONFIRM_BUTTON);
        final String confirmScript = intent
                .getStringExtra(DialogActivity.DIALOG_CONFIRM_SCRIPT);
        ...
        if (confirmTitle.trim().length() > 0) {
            builder = builder.setPositiveButton(confirmTitle,
                    new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            if (confirmScript != null
                                    && confirmScript.trim().length() > 0) {
                                Runnable r = new Runnable() {
                                    public void run() {
                                        try {
                                            Thread.sleep(500);
        ...
        String cancelTitle = intent
                .getStringExtra(DialogActivity.DIALOG_CANCEL_BUTTON);
        final String cancelScript = intent
                .getStringExtra(DialogActivity.DIALOG_CANCEL_SCRIPT);

        if (cancelTitle.trim().length() > 0) {
            builder = builder.setNegativeButton(cancelTitle,
                    new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            if (cancelScript != null
                                    && cancelScript.trim().length() > 0) {
                                Runnable r = new Runnable() {
                                    public void run() {
                                        try {
                                            Thread.sleep(500);
                ...
                .setOnDismissListener(new OnDismissListener() {
                    public void onDismiss(DialogInterface dialogInterface) {
                        ...
                        if (DialogActivity._pendingDialogs.size() > 0) {
                            ...
                            HashMap<String, Object> dialog = DialogActivity._pendingDialogs
                                    .remove(0);

                            Intent intent = new Intent();
                            intent.putExtra(DialogActivity.DIALOG_TITLE, dialog
                                    .get(DialogActivity.DIALOG_TITLE)
                                    .toString());
                            intent.putExtra(DialogActivity.DIALOG_MESSAGE,
                                    dialog.get(DialogActivity.DIALOG_MESSAGE)
                                            .toString());
                            intent.putExtra(
                                    DialogActivity.DIALOG_CONFIRM_BUTTON,
                                    dialog.get(
                            ...
                            intent.putExtra(
                                    ...
                                    dialog.get(
                                            DialogActivity.DIALOG_CANCEL_SCRIPT)
                                            .toString());
                            intent.putExtra(DialogActivity.DIALOG_TAG, dialog
                                    .get(DialogActivity.DIALOG_TAG).toString());
                            intent.putExtra(
                                    DialogActivity.DIALOG_PRIORITY,
                                    (Long) dialog
                                            .get(DialogActivity.DIALOG_PRIORITY));
    ...
    private static void sortPending() {
        Collections.sort(DialogActivity._pendingDialogs,
                new Comparator<HashMap<String, Object>>() {
                    public int compare(HashMap<String, Object> one,
                            HashMap<String, Object> two) {

                        String priorityOne = one.get(
                                DialogActivity.DIALOG_PRIORITY).toString();
                        String priorityTwo = two.get(
                                DialogActivity.DIALOG_PRIORITY).toString();
                        ...
                        Long addedOne = (Long) one
                                .get(DialogActivity.DIALOG_ADDED);
                        Long addedTwo = (Long) two
                                .get(DialogActivity.DIALOG_ADDED);
    ...
    public static void clearNativeDialogs(final Context context) {
        ...
        if (DialogActivity._currentDialog != null
                && DialogActivity._currentActivity != null) {
    ...
    public static void clearNativeDialogs(final Context context, String tag,
            HashMap<String, Object> replacement) {
        ...
        int stackCount = Integer.parseInt(prefs.getString(
                "config_dialog_count", "4"));

        while (DialogActivity._pendingDialogs.size() >= stackCount
                && stackCount > 0) {
            DialogActivity._pendingDialogs
                    .remove(DialogActivity._pendingDialogs.size() - 1);
        }
